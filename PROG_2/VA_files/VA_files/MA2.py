...
def assignment(wtok, variables: dict):
    ...
    while wtok.get_current() == '=':
        wtok.next()
        if wtok.get_current() in ("PI","E"):
            raise SyntaxError("Cannot change the value of constant")
        elif wtok.is_name():
...
def factor(wtok, variables):
    ...
    function_N = {'max': lambda *x: max(x), 'min': lambda *x: min(x), 
                  'sum': lambda *x: sum(x), 'mean': mean_value, 'std': standard_dev}
